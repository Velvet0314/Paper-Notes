...
5. 计算模型的输入/输出的维度
	...
	- 计算输入维度
		...
		3. 额外的图特征 —— `extra_features: __call__ function`
			...
			- 输出：一个 PlaceHolder，包含图的额外特征的对象，具有 X、E、y 三个属性
				- X：额外的节点特征
				- E：额外的边特征
				- y：额外的全局特征
			1. 计算图的大小比例
		...
		4. 分子的特征 —— `class ExtraMolecularFeatures: __call__ function`  
			...
			1. 计算形式电荷 ^extramolfeatures
				...
				class ChargeFeature:
					...
					计算形式电荷。
					原理：
					1. 根据边特征计算每个原子当前连接的总键级
					2. 根据节点特征确定原子类型对应的正常价态
					3. 电荷 = 正常价态 - 当前价态
					"""
					
					# 获取边类型数量，判断是否包含芳香键
					# E_t 最后一个维度是代表键的类型的 one-hot，这个 one-hot 的长度就表示有几种类型
					de = noisy_data["E_t"].shape[-1]
					
					if de == 5:
						# 包含芳香键：[无键, 单键, 双键, 三键, 芳香键]
					...
					# 计算加权边特征：E_t * bond_orders
					# 因为 bond_order.shape:[1, 1, 1, de]，相乘广播时只对最后一个维度做逐元素相乘，其他维度进行复制
					...
					# 构建原子类型到正常价态的映射
					# 例子：self.valencies = [4, 3, 2, 1, 1]，对应 [C, N, O, F, H]
					...
					# 形式电荷 = 正常价态 - 当前价态
					# tensor_a.type_as(tensor_b) 将 `tensor_a` 转换成与 `tensor_b` 相同的 dtype 和 device
			...
			3. 分子量归一化
				...
				class WeightFeature:
					def __init__(self, max_weight, atom_weights):
						# max_weight: 数据集中最大分子量，用于归一化
						self.max_weight = max_weight
						
						# atom_weights: 字典，原子类型 -> 原子量（如 {'C': 12, 'N': 14, 'O': 16}）
					...
					def __call__(self, noisy_data):
						...
						计算归一化分子量。
						步骤：
						1. 从 one-hot 编码中提取原子类型索引
						2. 根据索引查找原子量
						3. 求和得到分子量，除以最大分子量归一化
						"""
